...
function Avaliacoes() {
  ...
  const carregarAvaliacoes = async () => {
    try {
      const response = await axios.get(`http://localhost:8081/api/passeadores/${nomeUsuario}`);
      const passeador = response.data;
      const avaliacoes = passeador.avaliacoes?.$values || [];

      console.log("Avaliações do Passeador:", avaliacoes);
